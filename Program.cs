...
Console.WriteLine("Задача с семинара, сортируем нечётные столбцы:");
...
int[,] SortedOddRows(int[,] array)
{
    for (int i = 1; i < array.GetLength(1); i += 2)
    {
        ...
        {
            for (int l = k + 1, temp = 0; l < array.GetLength(0); l++)
            {
                if (array[k,i] > array[l,i])
                {
                    temp = array[k,i];
                    array[k,i] = array[l,i];
                    array[l,i] = temp;
...
/*Стандартный параметр ко всем задачам*/
